...
export const SyncCentralEditor: React.FC<SyncCentralEditorProps> = ({ 
  ...
  const documentationTopics = [
    { id: 1, title: "Sujet 1", status: "À réviser", version: "1.0", assignee: "Jean Dupont" },
    { id: 2, title: "Sujet 2", status: "À jour", version: "1.1", assignee: "Marie Martin" },
    { id: 3, title: "Sujet 3", status: "En cours", version: "1.2", assignee: "Pierre Durand" },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow p-6 overflow-auto">
        ...
        <div className="border p-4 h-64 overflow-auto" contentEditable>
          Entrez la description du correctif ou de l'évolution ici...
        </div>
      </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {documentationTopics.map((topic) => (
                <tr key={topic.id}>
                  <td className="border p-2">{selectedFeature ? features.find(f => f.id === selectedFeature)?.name : '-'}</td>
                  <td className="border p-2">
                    <a href="#" className="text-blue-600 hover:underline">{topic.title}</a>
                  </td>
                  <td className="border p-2">
                    <span className={`px-2 py-1 rounded-full text-sm ${
                      topic.status === "À réviser" ? "bg-orange-200 text-orange-800" :
                      topic.status === "À jour" ? "bg-green-200 text-green-800" :
                      topic.status === "En cours" ? "bg-blue-200 text-blue-800" :
                  ...
                  <td className="border p-2">
                    <Button size="sm" variant="outline" className="mr-2">Terminé</Button>
                    <Button size="sm" variant="outline">Marquer comme à jour</Button>
                  </td>
                </tr>
              ))}
            </tbody>
